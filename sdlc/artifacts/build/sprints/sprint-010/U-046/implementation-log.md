...
## Post-merge on-device remediation (W-1: toolbar height)

On-device validation revealed the code-review W-1 warning was real: with the Toolbar at a fixed `android:layout_height="?attr/actionBarSize"`, applying the status-bar top inset as `paddingTop` pushed the "SMS Backup+" title out of the visible toolbar area (title clipped/missing, though the status bar was correctly no longer overlapped). Fix applied in `app/src/main/res/layout/main.xml`: changed the Toolbar to `android:layout_height="wrap_content"` + `android:minHeight="?attr/actionBarSize"`, so the toolbar grows to `statusBarInset + actionBarSize` and the title renders below the status bar at full height. Re-verified on emulator-5554 (API 37): status-bar clock/icons fully visible, "SMS Backup+" title + overflow menu shown below the status bar, green toolbar still reaching the top edge (edge-to-edge look preserved), and BACKUP/RESTORE controls shifted down accordingly. assembleDebug green.
